...
namespace Banhang.Admin
{
    public partial class ChangePass : System.Web.UI.Page
    {
        ...
        protected void doipass_Click(object sender, EventArgs e)
        {
            if (passcu.Text != "")
            {
                Lpasscu.Visible = false;
            }
            if (passmoi.Text != "")
            {
                Lpassmoi.Visible = false;
            }
            if (passcu.Text.Equals(""))
            {
                Lpasscu.Visible = true;
                Lpasscu.Text = "Vui Lòng Nhập Password Cũ";
                passcu.Focus();
            }
            if (passmoi.Text.Equals(""))
            {
                Lpassmoi.Visible = true;
                Lpassmoi.Text = "Vui Lòng Nhập Password mới";
                passmoi.Focus();
            }
            else
            {
                ...
                {
                    using (var conn = new SqlDataAdapter("Select * from Admin where PassWord='" + passcu.Text.Trim() + "' ", cnn))
                    {
                        ...
                        if (data.Rows.Count > 0)
                        {
                                ...
                                string sql = "update Admin set PassWord=@pass where UserName=@username";
                                SqlCommand cmd = new SqlCommand(sql, cnn);
                                cmd.Parameters.AddWithValue("@username", id);
                                cmd.Parameters.AddWithValue("@pass", passmoi.Text.Trim());
                                cnn.Open();
                                cmd.ExecuteNonQuery();
                                cnn.Close();
                                Label2.Text = "Chúc Mừng Bạn Thay đổi Password Tài Khoản Thành Công";
                                Response.Redirect("~/Admin/Default.aspx"); ;                                                   
                        }
                        else
                        {
                            Lpasscu.Visible = true;
                            Lpasscu.Text = "Sai Password";
                        }
