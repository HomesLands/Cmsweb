...
{
    public partial class ThôngTinNSX : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                if (!IsPostBack)
                {
                    ...
                    load(Request.QueryString["idnsx"].ToString());
        ...
        protected void load(string mansx)
        {
            load1();
            string connectionstring = ConfigurationManager.ConnectionStrings["Tbecon"].ConnectionString;
            SqlConnection conn = new SqlConnection(connectionstring);
            string sql = "select  * from NhaSanXuat where MaSX=@mansx";
            ...
            while (reader.Read())
            {
                tenNSX.Text = reader["TenSX"].ToString();
        ...
        {
            load1();
            string mansx = Request.QueryString["idnsx"].ToString();
            string connectionstring = ConfigurationManager.ConnectionStrings["Tbecon"].ConnectionString;
            SqlConnection conn = new SqlConnection(connectionstring);
            SqlCommand cmd = new SqlCommand("Delete from NhaSanXuat where MaSX=@mansx", conn);
            cmd.Parameters.AddWithValue("@mansx", mansx);
            SqlDataAdapter adapter;
            adapter = new SqlDataAdapter(cmd);
            ...
            adapter.Fill(dt);
            adapter.Dispose();
            Label2.Text = "Xóa Nhà Cung Cấp Thành Công";
        ...
        protected void capnhat_Click(object sender, EventArgs e)
        {
            ...
            {
                Label3.Visible = true;
                Label3.Text = "Tên Nhà Cung Cấp Không Được Bỏ Trống";
                tenNSX.Focus();
            }
            ...
            else
            {
                ...
                string connectionstring = ConfigurationManager.ConnectionStrings["Tbecon"].ConnectionString;
                    SqlConnection conn = new SqlConnection(connectionstring);
                    string sql = "update NhaSanXuat set TenSX=@ten,DiaChi=@dc,DT=@dt,Email=@email where MaSX=@masx";
                    SqlCommand cmd = new SqlCommand(sql, conn);
                    cmd.Parameters.AddWithValue("@masx", mansx);
                    cmd.Parameters.AddWithValue("@ten", tenNSX.Text.Trim());
                    cmd.Parameters.AddWithValue("@dc", diachi.Text.Trim());
                    cmd.Parameters.AddWithValue("@dt", sdt.Text.Trim());
                    cmd.Parameters.AddWithValue("@email", email.Text.Trim());
                    conn.Open();
                    cmd.ExecuteNonQuery();
                    conn.Close();
                    Label2.Text = "Chúc Mừng Bạn Thay đổi Nhà Cung Cấp Thành Công";
                    Response.Redirect("~/Admin/AddNhaSanXuat.aspx");
            }               
        }
    }
