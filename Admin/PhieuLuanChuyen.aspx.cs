...
{
    public partial class PhieuLuanChuyen : System.Web.UI.Page
    {
        ...
        protected void load()
        {
            ...
            string connectionstring = ConfigurationManager.ConnectionStrings["Tbecon"].ConnectionString;
            SqlConnection conn = new SqlConnection(connectionstring);
            SqlCommand cmd = new SqlCommand("SELECT DISTINCT Mahoadon,Date from Luanchuyen", conn);
            SqlDataAdapter adapter;
            adapter = new SqlDataAdapter(cmd);
            DataSet dt = new DataSet();
            adapter.Fill(dt, "Luanchuyen");
            Repeater1.DataSource = dt.Tables["Luanchuyen"];
            Repeater1.DataBind();
        }
        private void LoadDataToDropDownList1()
        {
            ...
            SqlDataAdapter adapter = new SqlDataAdapter("Select Madt,Tenkho,DiaChi From DiachiKho", connectionstring);
            adapter.Fill(dataTable);
            diachicu.DataSource = dataTable;
            diachicu.DataTextField = "DiaChi"; 
            diachicu.DataValueField = "DiaChi"; 
            diachicu.DataBind();

            Tenkhocu.DataSource = dataTable;
            Tenkhocu.DataTextField = "Tenkho";
            Tenkhocu.DataValueField = "Tenkho";
            Tenkhocu.DataBind();

            diachimoi.DataSource = dataTable;
            diachimoi.DataTextField = "DiaChi"; 
            diachimoi.DataValueField = "DiaChi"; 
            diachimoi.DataBind();

            Tenkhomoi.DataSource = dataTable;
            Tenkhomoi.DataTextField = "Tenkho";
            Tenkhomoi.DataValueField = "Tenkho";
            Tenkhomoi.DataBind();
        }
        ...
        protected void gui_Click(object sender, EventArgs e)
        {
            ...
            if (tieude.Text != "")
            {
                Ltieude.Visible = false;
            }
            if (Lydokiem.Text != "")
            {
                LLydokiem.Visible = false;
            }
            if (Nguoidexuat.Text != "")
            {
                LNguoidexuat.Visible = false;
            }
            if (madon.Text.Equals(""))
            {
                Lmadon.Visible = true;
                Lmadon.Text = "Mã Phiếu Không Được Bỏ Trống";
                madon.Focus();
            }
            if (tieude.Text.Equals(""))
            {
                Ltieude.Visible = true;
                Ltieude.Text = "Tiêu Đề Không Được Bỏ Trống";
                tieude.Focus();
            }
            if (Nguoidexuat.Text.Equals(""))
            {
                LNguoidexuat.Visible = true;
                LNguoidexuat.Text = "Người Đề Xuất Không Được Bỏ Trống";
                Nguoidexuat.Focus();
            }
            if (Lydokiem.Text.Equals(""))
            {
                LLydokiem.Visible = true;
                LLydokiem.Text = "Lý Do Không Được Bỏ Trống";
                Lydokiem.Focus();
            }
            else
            {
                ...
                {
                    SqlCommand cmd = new SqlCommand("select * from Luanchuyen where Mahoadon='" + madon.Text.Trim() + "' ", cnn);
                    SqlDataAdapter adapter;
                    adapter = new SqlDataAdapter(cmd);
                    DataSet dt = new DataSet();
                    adapter.Fill(dt, "Luanchuyen");
                    ...
                    {
                        Label2.Text = "Trùng Mã Cho Mượn Luân Chuyễn! Vui Lòng Chọn Lại Mã";
                    }else
                    {
                        string date = DateTime.Now.ToString("ddMMyyyy-HHmm");
                        madon.Text = "LC" + date;
                        using (var conn = new SqlDataAdapter("Insert Luanchuyen values('" + madon.Text.Trim() + "',N'" + tieude.Text.Trim() + "',N'" + tencty.Text.Trim() + "',N'" + diachicty.Text.Trim() + "',N'" + mucdich.Text.Trim() + "',N'" + Lydokiem.Text.Trim() + "','" + datet + "',N'" + thoigianmuonluanchuyen.Text.Trim() + "',N'" + Tenkhocu.Text.Trim() + "',N'" + diachicu.Text.Trim() + "',N'" + Nguoidexuat.Text.Trim() + "',N'" + Tenkhomoi.Text.Trim() + "',N'" + diachimoi.Text.Trim() + "',1,'mamacdinh','','','')", cnn))
                        {
                            var data = new DataTable();
                            conn.Fill(data);
                            Label2.Text = "Chúc Mừng Bạn Thêm Phiếu Thành Công";
                            load();
                            Response.Redirect("XuatLuanChuyen.aspx?idnsx=" + madon.Text.Trim());
                        }
                    }
                }                  
              }                         
              ...
              using (var cnn = new SqlConnection(System.Web.Configuration.WebConfigurationManager.ConnectionStrings["Tbecon"].ToString()))
                    {
                        ...
                        using (var conn2 = new SqlDataAdapter("Delete from Luanchuyen where MaSP='" + ID1 + "' ", cnn))
                            {
                                var data = new DataTable();
                                conn2.Fill(data);
                            }
                    }
                    Label1.Text = "Xóa Đơn Rác Thành Công";
                    load(); 
              }
    }
    }
