...
namespace TBE.Admin
{
    public partial class PXKTKDD : System.Web.UI.Page
    {
        ...
        public void load1()
        {
            ...
            else if(Label3.Text.Equals("muahang") || Label3.Text.Equals("it") || Label3.Text.Equals("thuquy") || Label3.Text.Equals("nvhcns") || Label3.Text.Equals("nvpda"))
            {
                Response.Redirect("~/Admin/Chantruycap.aspx");
            }
            ...
            else if (Label3.Text.Equals("kho")|| Label3.Text.Equals("giamdoc"))
            {
                var sqlcon = new SqlConnection(ConfigurationManager.ConnectionStrings["Tbecon"].ToString());
                var sqlda = new SqlDataAdapter("SELECT * from Xuatkho  where StatusPXK=N'Giao Thành Công' ORDER BY Date DESC ", sqlcon);
                var dt1 = new DataTable();
                sqlda.Fill(dt1);
                lvCustomers.DataSource = dt1;
                lvCustomers.DataBind();
            }
            else if (Label3.Text.Equals("giaonhan"))
            {
                var sqlcon = new SqlConnection(ConfigurationManager.ConnectionStrings["Tbecon"].ToString());
                var sqlda = new SqlDataAdapter("SELECT * from Xuatkho  where StatusPXK=N'Giao Thành Công' and Nguoigiaohanh=N'"+ hotenad.Text + "' ORDER BY Date DESC ", sqlcon);
                var dt1 = new DataTable();
                sqlda.Fill(dt1);
                lvCustomers.DataSource = dt1;
                lvCustomers.DataBind();
            }
            else if (Label3.Text.Equals("thukho"))
            {
                var sqlcon = new SqlConnection(ConfigurationManager.ConnectionStrings["Tbecon"].ToString());
                var sqlda = new SqlDataAdapter("SELECT * from Xuatkho  where StatusPXK=N'Giao Thành Công' and Thukho=N'" + hotenad.Text + "' ORDER BY Date DESC ", sqlcon);
                var dt1 = new DataTable();
                sqlda.Fill(dt1);
                lvCustomers.DataSource = dt1;
                lvCustomers.DataBind();
