...
{
    public partial class TtAddhoadon : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                if (!IsPostBack)
                {
                    ...
                    load1(Request.QueryString["Madon"].ToString());
        ...
        protected void load1(string madon)
        {
            maxuat.Enabled = false;
            tencongty.Enabled = false;
            Congtrinhxuat.Enabled = false;
            Nguoinhan.Enabled = false;
            Noixuatkho.Enabled = false;
            ngayxuat.Enabled = false;
            Lydoxuat.Enabled = false;
            ...
            while (reader.Read())
            {
                maxuat.Text = reader["Mahoadon"].ToString();
                tencongty.Text = reader["Tencongty"].ToString();
                ngayxuat.Text = reader["Date"].ToString();
                Congtrinhxuat.Text = reader["Congtrinhxuat"].ToString();
                Nguoinhan.Text = reader["Nguoinhan"].ToString();
                Noixuatkho.Text = reader["Noixuatkho"].ToString();
                Lydoxuat.Text = reader["Lydoxuat"].ToString();
                Sochungtu.Text = reader["Sochungtu"].ToString();
            }
            reader.Close();
            conn.Close();
        }
        ...
        protected void capnhat_Click(object sender, EventArgs e)
        {
            ...
            string madon = Request.QueryString["Madon"].ToString();
            String datet = DateTime.Now.ToString("yyyy-MM-ddTHH:mm:ss");
                ...
                {                       
                        int a = Int32.Parse(Soluong.Text.Trim());
                        int b = Int32.Parse(slton.Text.Trim());
                        int tong = b - a;
                        if (b == 0)
                        {
                            Label2.Text = "Kho Hết Sản Phẩm";
                        }
                        else if(b<a)
                        {
                            Label2.Text = "Số Lượng Trong Kho Không Đủ Xuất Kho! Vui Lòng Chọn Lại";
                        }
                        else
                        {
                            using (var conn = new SqlDataAdapter("Insert Xuatkho values('" + madon + "',N'" + tencongty.Text.Trim() + "','" + datet + "','" + Soluong.Text.Trim() + "','" + Soluong.Text.Trim() + "',N'" + diachi.Text.Trim() + "',N'" + Congtrinhxuat.Text.Trim() + "',N'" + Nguoinhan.Text.Trim() + "',N'" + Noixuatkho.Text.Trim() + "','" + Lydoxuat.Text.Trim() + "',N'" + Sochungtu.Text.Trim() + "','" + masp + "','" + RFID.Text.Trim() + "')", cnn))
                            {
                                var data = new DataTable();
                                conn.Fill(data);
                            }
                            using (var conn = new SqlDataAdapter("Update SanPhamTBE set SLTon='" + tong + "' where MaSP='" + masp + "'", cnn))
                            {
                                var data = new DataTable();
                                conn.Fill(data);
                            }
                            Label2.Text = "Chúc Mừng Bạn Thêm Phiếu Xuất Kho Thành Công";
                            Response.Redirect("Viewphieuthu.aspx?idnsx=" + madon); 
                        }
                }         
          }
    }
