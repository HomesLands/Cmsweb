...
{
    public partial class ThôngTinDiachi : System.Web.UI.Page
    {
        ...
        protected void load(string madc)
        {
            load1();
            string connectionstring = ConfigurationManager.ConnectionStrings["Tbecon"].ConnectionString;
            SqlConnection conn = new SqlConnection(connectionstring);
            string sql = "select  * from DiachiKho where Madt=@madc";
            ...
            while (reader.Read())
            {
                tenkho.Text = reader["Tenkho"].ToString();
        ...
        {
            load1();
            string madc = Request.QueryString["idnsx"].ToString();
            string connectionstring = ConfigurationManager.ConnectionStrings["Tbecon"].ConnectionString;
            SqlConnection conn = new SqlConnection(connectionstring);
            SqlCommand cmd = new SqlCommand("Delete from DiachiKho where Madt=@madc", conn);
            cmd.Parameters.AddWithValue("@madc", madc);
            SqlDataAdapter adapter;
            adapter = new SqlDataAdapter(cmd);
            ...
            adapter.Fill(dt);
            adapter.Dispose();
            Label2.Text = "Xóa Địa Chỉ Thành Công";
        ...
        protected void capnhat_Click(object sender, EventArgs e)
        {
            ...
            if (tenkho.Text != "")
            {
                Ltenkho.Visible = false;
            }
            if (diachi.Text != "")
            {
                Ldiachi.Visible = false;
            }
            if (tenkho.Text.Equals(""))
            {
                Ltenkho.Visible = true;
                Ltenkho.Text = "Tên Địa Chỉ Không Được Bỏ Trống";
                tenkho.Focus();
            }
            if (diachi.Text.Equals(""))
            {
                Ldiachi.Visible = true;
                Ldiachi.Text = "Địa Chỉ Không Được Bỏ Trống";
                diachi.Focus();
            }
            else
            {
                ...
                string connectionstring = ConfigurationManager.ConnectionStrings["Tbecon"].ConnectionString;
                    SqlConnection conn = new SqlConnection(connectionstring);
                    string sql = "update DiachiKho set Tenkho=@ten,Diachi=@dc where Madt=@madc";
                    SqlCommand cmd = new SqlCommand(sql, conn);
                    cmd.Parameters.AddWithValue("@madc", madc);
                    cmd.Parameters.AddWithValue("@ten", tenkho.Text.Trim());
                    cmd.Parameters.AddWithValue("@dc", diachi.Text.Trim());
                    conn.Open();
                    cmd.ExecuteNonQuery();
                    conn.Close();
                    Label2.Text = "Chúc Mừng Bạn Thay đổi Địa Chỉ Thành Công";
                    Response.Redirect("~/Admin/AddDiaChi.aspx");
            }               
        }
    }
