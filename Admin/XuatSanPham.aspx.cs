...
{
    public partial class XuatSanPham : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    load1();
                    Label27.Text = "Welcome To " + " " + Session["ADminID"];
                    load();
                    txt_autocomplete.Attributes.Add("onkeypress", "return clickButton(event,'" + Button3.ClientID + "')");
                    txt_autocomplete.Visible = false;
        ...
        protected void load()
        {
            ...
            string sql = "select  * from Xuatkho where Mahoadon='" + madon + "'";
            SqlCommand cmd1 = new SqlCommand(sql, conn1);
            ...
            while (reader.Read())
            {
                string ipsp = reader["MaSP"].ToString();
                if (ipsp.Equals("mamacdinh"))
                {                
                }
                else
                {
                    SqlConnection conn = new SqlConnection(connectionstring);
                    SqlCommand cmd = new SqlCommand("SELECT * from Xuatkho  where Mahoadon='" + madon + "' and MaSP!='mamacdinh' ", conn);
                    SqlDataAdapter adapter;
                    adapter = new SqlDataAdapter(cmd);
                    DataSet dt = new DataSet();
                    adapter.Fill(dt, "Xuatkho");
                    Repeater2.DataSource = dt.Tables["Xuatkho"];
                    Repeater2.DataBind();
                }
            }
            reader.Close();
        ...
        public void load1()
        {
            ...
            while (reader.Read())
            {
                Label28.Text = reader["Manager"].ToString();
                hotenad.Text = reader["HoTen"].ToString();
            }
            reader.Close();
            conn1.Close();
            if (Label28.Text.Equals("khach"))
            {
                Response.Redirect("~/Admin/Chantruycap.aspx");
            }
            if (Label28.Text.Equals("ketoan"))
            {
                Response.Redirect("~/Admin/Chantruycap.aspx");
            }
            if (Label28.Text.Equals("muahang") || Label28.Text.Equals("giaonhan") || Label28.Text.Equals("it") || Label28.Text.Equals("thukho") || Label28.Text.Equals("thuquy") || Label28.Text.Equals("nvhcns") || Label28.Text.Equals("nvpda"))
            {
                Response.Redirect("~/Admin/Chantruycap.aspx");
            }
            if (Label28.Text.Equals("chihuytruong"))
            {
                Response.Redirect("~/Admin/Chantruycap.aspx");
            }
            if (Label28.Text.Equals("ppda") || Label28.Text.Equals("tpkehoach") || Label28.Text.Equals("tpda") || Label28.Text.Equals("tpmh") || Label28.Text.Equals("ktt") || Label28.Text.Equals("tpns"))
            {
                Response.Redirect("~/Admin/Chantruycap.aspx");
            }
            if (Label28.Text.Equals("nhanvien") || Label28.Text.Equals("gsat") || Label28.Text.Equals("gsd") || Label28.Text.Equals("gsck") || Label28.Text.Equals("gs"))
            {
                Response.Redirect("~/Admin/Chantruycap.aspx");
            }
            if (Label28.Text.Equals("thohan") || Label28.Text.Equals("thocokhi") || Label28.Text.Equals("baotridien") || Label28.Text.Equals("baotricokhi") || Label28.Text.Equals("QSQC"))
            {
                Response.Redirect("~/Admin/Chantruycap.aspx");
            }
        }
        ...
        protected void Button3_Click(object sender, EventArgs e)
        {
                ...
                string connectionstring = ConfigurationManager.ConnectionStrings["Tbecon"].ConnectionString;
                SqlConnection conn = new SqlConnection(connectionstring);
                SqlCommand cmd = new SqlCommand("select * from SanPhamTBE where MaRFID='" + marfid.Text.Trim() + "' and trangthai='action' ", conn);
                SqlDataAdapter adapter;
                adapter = new SqlDataAdapter(cmd);
                DataSet dt = new DataSet();
                adapter.Fill(dt, "SanPhamTBE");

                string sql = "select  * from SanPhamTBE where MaRFID=@marfid ";
                SqlCommand cmd1 = new SqlCommand(sql, conn);
                cmd1.Parameters.AddWithValue("@marfid", marfid.Text.Trim());
                conn.Open();
                SqlDataReader reader = cmd1.ExecuteReader();
                while (reader.Read())
                {
                    txt_autocomplete.Text = reader["MaSP"].ToString();
                    Label8.Text = reader["TenSP"].ToString();
                    Label12.Text = reader["DonGia"].ToString();
                    Label24.Text = reader["MaRFID"].ToString();
                    Label25.Text = reader["trangthai"].ToString();
                }
                reader.Close();
                conn.Close();
                string madon = Request.QueryString["idnsx"].ToString();
                string sql1 = "select  * from Xuatkho where Mahoadon=@madon";
                ...
                while (reader1.Read())
                {
                    Label13.Text = reader1["Mahoadon"].ToString();
                    Label16.Text = reader1["Tencongty"].ToString();
                    Label17.Text = reader1["Date"].ToString();
                    Label18.Text = reader1["Congtrinhxuat"].ToString();
                    Label19.Text = reader1["Nguoinhan"].ToString();
                    Label20.Text = reader1["Noixuatkho"].ToString();
                    Label21.Text = reader1["StatusPXK"].ToString();
                    Label22.Text = reader1["Sochungtu"].ToString();
                    Label23.Text = reader1["Diachi"].ToString();
                }
                reader1.Close();
                conn.Close();
                 if(marfid.Text!="")
                 {
                     ...
                     {
                         if (Label25.Text.Equals("disable"))
                         {
                             Label3.Visible = true;
                             Label3.Text = "Mã RFID Không Tồn Tại";
                             marfid.Text = "";
                             marfid.Focus();
                         }
                         else
                         {
                             if (marfid.Text.Length != 22 || marfid.Text.IndexOf("E") > 0)
                             {
                                 Label8.Text = "Định dạng mã RFID sai";
                                 marfid.Text = "";
                                 marfid.Focus();
                             }
                             else
                             {
                                 using (var conn1 = new SqlDataAdapter("Insert Xuatkho values('" + madon + "',N'" + Label16.Text + "','" + Label17.Text + "',1,1,N'" + Label23.Text + "',N'" + Label18.Text + "',N'" + Label19.Text + "',N'" + Label20.Text + "','" + Label21.Text + "',N'" + Label22.Text + "','" + txt_autocomplete.Text.Trim() + "','" + marfid.Text.Trim() + "',N'" + Label8.Text.Trim() + "',N'" + Label12.Text.Trim() + "')", conn))
                                 {
                                     var data = new DataTable();
                                     conn1.Fill(data);
                                 }
                                 //string ID1 = "mamacdinh";
                                 //using (var conn2 = new SqlDataAdapter("Delete from Xuatkho where MaSP='" + ID1 + "' ", conn))
                                 //{
                                 //    var data = new DataTable();
                                 //    conn2.Fill(data);
                                 //}
                                 using (var conn2 = new SqlDataAdapter("update SanPhamTBE set trangthai='disable' where MaRFID='" + marfid.Text.Trim() + "' ", conn))
                                 {
                                     var data = new DataTable();
                                     conn2.Fill(data);
                                 }
                 ...
                 {
                    Label3.Text = "Vui Lòng Điền Mã RFID !!!!";
                    marfid.Text = "";
                    marfid.Focus();
                 }  
        }
        ...
        {
            load1();
            string masp = Request.QueryString["idnsx"].ToString();
            Response.Redirect("TtAddhoadon.aspx?idSP=" + e.CommandArgument + "&Madon=" + masp);         
        }
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            load1();
            string masp = Request.QueryString["idnsx"].ToString();
            Response.Redirect("Viewphieuthu.aspx?idnsx=" + masp);  
        }
        ...
        protected void Button4_Click(object sender, EventArgs e)
        {
                ...
                while (reader1.Read())
                {
                    Label26.Text = reader1["MaRFID"].ToString();               
                }
                reader1.Close();
                conn.Close();

                    SqlCommand cmd = new SqlCommand("select * from Xuatkho where Mahoadon='" + madon + "' ", conn);
                    SqlDataAdapter adapter;
                    adapter = new SqlDataAdapter(cmd);
                    DataSet dt = new DataSet();
                    adapter.Fill(dt, "SanPhamTBE");
                    ...
                    //{
                        using (var conn2 = new SqlDataAdapter("Delete from Xuatkho where Mahoadon='" + madon + "' and MaRFID='" + Label26.Text + "' ", conn))
                        {
                            var data = new DataTable();
                            conn2.Fill(data);
                        }
                        using (var conn3 = new SqlDataAdapter("update SanPhamTBE set trangthai='action' where MaRFID='" + Label26.Text + "' ", conn))
                        {
                            var data = new DataTable();
                            conn3.Fill(data);
                            Response.Redirect("XuatSanPham.aspx?idnsx=" + madon);
                            load();
                        }
                    //}
                    //else
                    //{
                    //    Label3.Visible = true;
                    //    Label3.Text = "Vui Lòng Không Xóa Hết Sản Phẩm! Hãy Thêm RFID Khác Trước Khi Xóa RFID Này ";
                    //    marfid.Text = "";
                    //    marfid.Focus();
                    //}
            }
