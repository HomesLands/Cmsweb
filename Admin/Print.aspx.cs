...
namespace TBE.Admin
{
    public partial class Print : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                if (!IsPostBack)
                {
                    ...
                    pager.DataSource = load().DefaultView;
        ...
        public DataTable load()
        {
            ...
            {
                cnn.Open();
                string sql1 = "Select * from SanPhamTBE where trangthai='action'";
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            string connectionstring = ConfigurationManager.ConnectionStrings["Tbecon"].ConnectionString;
            SqlConnection conn = new SqlConnection(connectionstring);
            SqlCommand cmd = new SqlCommand("select * from SanPhamTBE where TenSP like N'%" + tensp.Text.Trim() + "%' and trangthai='action' ", conn);
            SqlDataAdapter adapter;
            adapter = new SqlDataAdapter(cmd);
            DataSet dt = new DataSet();
