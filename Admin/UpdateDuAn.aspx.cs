...
namespace TBE.Admin
{
    public partial class UpdateDuAn : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                if (!IsPostBack)
                {
                    ...
                    load1();
                    Label15.Text = "Welcome To " + " " + Session["ADminID"];
                    ...
                    anhload.Visible = false;
                    anhload1.Visible = false;
                    kho1.Visible = false;
                    kho2.Visible = false;
                    kho3.Visible = false;
                    kho4.Visible = false;
        ...
        public static List<string> GetEmployeeName(string empName)
        {
            ...
            using (var con = new SqlConnection(System.Web.Configuration.WebConfigurationManager.ConnectionStrings["Tbecon"].ToString()))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    cmd.CommandText = "select Top 10 HoTen from Admin where HoTen LIKE '%'+@SearchEmpName+'%'";
                    cmd.Connection = con;
                    con.Open();
                    ...
                    while (dr.Read())
                    {
                        empResult.Add(dr["HoTen"].ToString());
                    }
                    con.Close();
                    return empResult;
                }
        ...
        protected void load()
        {
            ...
            while (reader.Read())
            {
                TenDuAn.Text = reader["TenDuAn"].ToString();
                Tiendo.Text = reader["TienDo"].ToString();
                NgayBatDau.Text = reader["DateStart"].ToString();
                NgayKetThuc.Text = reader["DateEnd"].ToString();
                txtEmpName.Text = reader["NguoiQuanLy"].ToString();
                txtEmpName1.Text = reader["NguoiGiamSat"].ToString();
                txtEmpName2.Text = reader["NguoiThucHien"].ToString();
                Ghichu.Text = reader["MoTa"].ToString();
            }
            reader.Close();
            conn.Close();
        }
        ...
        public void load1()
        {
            ...
            if (Label16.Text.Equals("muahang") || Label16.Text.Equals("it") || Label16.Text.Equals("thukho") || Label16.Text.Equals("thuquy") || Label16.Text.Equals("nvhcns"))
            {
                Response.Redirect("~/Admin/Chantruycap.aspx");
            }
        ...
        protected void gui_Click(object sender, EventArgs e)
        {
            ...
            if (TenDuAn.Text != "")
            {
                LTenDuAn.Visible = false;
            }
            if (TenDuAn.Text.Equals(""))
            {
                LTenDuAn.Visible = true;
                LTenDuAn.Text = "Tên Dự Án Không Được Bỏ Trống";
                TenDuAn.Focus();
            }
            else
            {
                using (var cnn = new SqlConnection(System.Web.Configuration.WebConfigurationManager.ConnectionStrings["Tbecon"].ToString()))
                {
                    ...
                    {
                        String id = Request.QueryString["idad"].ToString();
                        string fileName = "../FileUp/" + FileUpload1.FileName;
                        string filePath = MapPath(fileName);
                        FileUpload1.SaveAs(filePath);
                        string connectionstring = ConfigurationManager.ConnectionStrings["Tbecon"].ConnectionString;
                        SqlConnection conn = new SqlConnection(connectionstring);
                        string sql = "update Duan set TenDuAn=@tenduan,DateStart=@ngaybatdau,DateEnd=@ngaykethuc,NguoiQuanLy=@nguoiquanly,NguoiGiamSat=@nguoigiamsat,NguoiThucHien=@nguoithuchien,TienDo=@tiendo,MoTa=@mota,Upload=@upload where MaDuAn=@user";
                        SqlCommand cmd = new SqlCommand(sql, conn);
                        cmd.Parameters.AddWithValue("@user", id);
                        cmd.Parameters.AddWithValue("@tenduan", TenDuAn.Text.Trim());                   
                        cmd.Parameters.AddWithValue("@ngaybatdau", NgayBatDau.Text.Trim());
                        cmd.Parameters.AddWithValue("@ngaykethuc", NgayKetThuc.Text.Trim());
                        cmd.Parameters.AddWithValue("@nguoiquanly", txtEmpName.Text.Trim());
                        cmd.Parameters.AddWithValue("@nguoigiamsat", txtEmpName1.Text.Trim());
                        cmd.Parameters.AddWithValue("@nguoithuchien", txtEmpName2.Text.Trim());
                        cmd.Parameters.AddWithValue("@tiendo", Tiendo.Text.Trim());
                        cmd.Parameters.AddWithValue("@mota", Ghichu.Text.Trim());
                        cmd.Parameters.AddWithValue("@upload", fileName);
                        conn.Open();
                        cmd.ExecuteNonQuery();
                        conn.Close();
                        Label2.Text = "Chúc Mừng Bạn Cập Nhật Dự Án Thành Công";
                    }
                    else
                    {
                        String id = Request.QueryString["idad"].ToString();
                        string connectionstring = ConfigurationManager.ConnectionStrings["Tbecon"].ConnectionString;
                        SqlConnection conn = new SqlConnection(connectionstring);
                        string sql = "update Duan set TenDuAn=@tenduan,DateStart=@ngaybatdau,DateEnd=@ngaykethuc,NguoiQuanLy=@nguoiquanly,NguoiGiamSat=@nguoigiamsat,NguoiThucHien=@nguoithuchien,TienDo=@tiendo,MoTa=@mota where MaDuAn=@user";
                        SqlCommand cmd = new SqlCommand(sql, conn);
                        cmd.Parameters.AddWithValue("@user", id);
                        cmd.Parameters.AddWithValue("@tenduan", TenDuAn.Text.Trim());
                        cmd.Parameters.AddWithValue("@ngaybatdau", NgayBatDau.Text.Trim());
