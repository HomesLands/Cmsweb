...
namespace Banhang.Admin
{
    public partial class ThongtinSanPham : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                if (!IsPostBack)
                {
                    ...
                    load(Request.QueryString["idSP"].ToString());
        ...
        protected void load(string masp)
        {
            ...
            string connectionstring = ConfigurationManager.ConnectionStrings["Tbecon"].ConnectionString;
            SqlConnection conn = new SqlConnection(connectionstring);
            string sql = "select  * from SanPhamTBE where MaSP=@masp and trangthai='action' ";
            ...
            while (reader.Read())
            {
               ...
               // RFID.Text = reader["MaRFID"].ToString();                
                MaModel.Text = reader["MaModel"].ToString();
                tensp.Text = reader["TenSP"].ToString();
                Label3.Text = reader["NgayNhap"].ToString();
        ...
        {
            load1();
            string masp = Request.QueryString["idSP"].ToString();
            string connectionstring = ConfigurationManager.ConnectionStrings["Tbecon"].ConnectionString;
            SqlConnection conn = new SqlConnection(connectionstring);
            SqlCommand cmd = new SqlCommand("Delete from SanPhamTBE where MaSP=@masp", conn);
            cmd.Parameters.AddWithValue("@masp", masp);
            SqlDataAdapter adapter;
            adapter = new SqlDataAdapter(cmd);
            ...
            adapter.Fill(dt);
            adapter.Dispose();
            Label2.Text = "Xóa Sản Phẩm Thành Công";
        ...
        protected void capnhat_Click(object sender, EventArgs e)
        {
            ...
            if (slton.Text != "")
            {
                Lslton.Visible = false;
            }
            if (MaModel.Text != "")
            {
                LMaModel.Visible = false;
            }
            if (ghichu.Text != "")
            {
            ...
            {
                LRFID.Visible = true;
                LRFID.Text = "RFID Không Được Bỏ Trống";
                RFID.Focus();
            }
            if (slton.Text.Equals(""))
            {
                Lslton.Visible = true;
                Lslton.Text = "Số Lượng Tồn Không Được Bỏ Trống";
                slton.Focus();
            }
            ...
            if (MaModel.Text.Equals(""))
            {
                LMaModel.Visible = true;
                LMaModel.Text = "Mã Model/Serial Không Được Bỏ Trống";
                MaModel.Focus();
            }         
            else
            {                          
                    ...
                    using (Bitmap bitMap = new Bitmap(barCode.Length * 40, 80))
                    {
                        using (Graphics graphics = Graphics.FromImage(bitMap))
                        {
                            ...
                            bitMap.Save(Server.MapPath("~/Data Image/" + masp + ".png"));
                            string Image1 = "/Data Image/" + masp + ".png";
                            //End QRcode
                            string connectionstring = ConfigurationManager.ConnectionStrings["Tbecon"].ConnectionString;
                            SqlConnection conn = new SqlConnection(connectionstring);
                            string sql = "update SanPhamTBE set MaSX=@masx,MaRFID=@rfid,MaModel=@mamodel,TenSP=@ten,NgayNhap=@ngaynhap,SLTon=@slTon,DonGia=@dongia,Diachi=@diachi,QRcode=@qrcode,GhiChu=@ghichu where MaSP=@masp";
                            SqlCommand cmd = new SqlCommand(sql, conn);
                            cmd.Parameters.AddWithValue("@masp", masp);
                            cmd.Parameters.AddWithValue("@masx", DropDownList1.Text);
                            cmd.Parameters.AddWithValue("@rfid", RFID.Text);
                            cmd.Parameters.AddWithValue("@mamodel", MaModel.Text);
                            cmd.Parameters.AddWithValue("@ten", tensp.Text);
                            cmd.Parameters.AddWithValue("@ngaynhap", date);
                            cmd.Parameters.AddWithValue("@slTon", slton.Text);
                            cmd.Parameters.AddWithValue("@dongia", dongia.Text);
                            cmd.Parameters.AddWithValue("@diachi", diachi.Text);
                            cmd.Parameters.AddWithValue("@qrcode", masp);
                            cmd.Parameters.AddWithValue("@ghichu", ghichu.Text);
                            conn.Open();
                            cmd.ExecuteNonQuery();
                            conn.Close();
                            Label2.Text = "Chúc Mừng Bạn Thay đổi Thông Tin Sản Phẩm Thành Công";
                            Response.Redirect("~/Admin/Default.aspx");
                        }
                    }                               
                }        
