...
{
    public partial class themxuatkho : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                if (!IsPostBack)
                {
                    ...
                    madon.Text = Session["ADminID1"].ToString().Trim();
        ...
        {           
                ...
                else
                {
                    ...
                    using (var conn = new SqlDataAdapter("Insert vattuyeucau values('" + b + "',N'" + TextBox5.Text.Trim() + "',N'" + TextBox4.Text.Trim() + "',N'" + TextBox1.Text.Trim() + "',N'" + TextBox3.Text.Trim() + "',N'" + TextBox2.Text.Trim() + "',N'" + TextBox2.Text.Trim() + "',N'" + TextBox6.Text.Trim() + "',N'" + serialnumber.Text.Trim() + "',N'" + Label32.Text.Trim() + "')", cnn))
                        {                            
                            SqlCommand cmd = new SqlCommand("select * from vattuyeucau where MaSP='" + TextBox5.Text.Trim() + "' and Mahoadon='" + b + "'  ", cnn);
                            SqlDataAdapter adapter;
                            adapter = new SqlDataAdapter(cmd);
                            DataSet dt = new DataSet();
        ...
        protected void load()
        {
            ...
            string connectionstring = ConfigurationManager.ConnectionStrings["Tbecon"].ConnectionString;            
                SqlConnection conn = new SqlConnection(connectionstring);
                SqlCommand cmd = new SqlCommand("SELECT * from vattuyeucau  where Mahoadon='" + b + "' ", conn);
                SqlDataAdapter adapter;
                adapter = new SqlDataAdapter(cmd);
                DataSet dt = new DataSet();
                adapter.Fill(dt, "vattuyeucau");
        ...
        protected void lnkCustomize_Command(object sender, CommandEventArgs e)
        {
            ...
            string sql = "select  * from Phieuycvt where Mahoadon='" + a + "' ";
            SqlCommand cmd1 = new SqlCommand(sql, conn1);
            conn1.Open();
            SqlDataReader reader = cmd1.ExecuteReader();
            while (reader.Read())
        ...
        protected void Button3_Click(object sender, EventArgs e)
        {
            ...
            using (var cnn = new SqlConnection(System.Web.Configuration.WebConfigurationManager.ConnectionStrings["Tbecon"].ToString()))
            {
                using (var conn = new SqlDataAdapter("Select * from vattuyeucau where Masp='" + TextBox5.Text.Trim() + "' and Mahoadon='" + b + "'   ", cnn))
                {
                    var data = new DataTable();
                    conn.Fill(data);                      
                        string sql = "update vattuyeucau set Soluong=@sl where Masp=@masp and Mahoadon=@mahd";
                        SqlCommand cmd = new SqlCommand(sql, cnn);
                        cmd.Parameters.AddWithValue("@masp", TextBox5.Text.Trim());
                        cmd.Parameters.AddWithValue("@sl", TextBox2.Text.Trim());
                        cmd.Parameters.AddWithValue("@mahd", madon.Text.Trim());
                        cnn.Open();
                        cmd.ExecuteNonQuery();
                        cnn.Close();
                        load();
                        ...
                        TextBox6.Text = "";
                        Label2.Text = "";
                    Label2.Text = "Chúc Mừng Bạn Thay đổi Số Lượng Sản Phẩm Thành Công";                           
                }
            }
        }
        ...
        protected void lnkCustomize_Command1(object sender, CommandEventArgs e)
        {
            string connectionstring = ConfigurationManager.ConnectionStrings["Tbecon"].ConnectionString;
            SqlConnection conn1 = new SqlConnection(connectionstring);
            string sql = "select  * from SanPhamTBE where Masp='" + e.CommandArgument.ToString() + "'  ";
            SqlCommand cmd1 = new SqlCommand(sql, conn1);
        ...
        {
            string eeer = Request.QueryString["idnsx"].ToString();
            Response.Redirect("~/Admin/Viewcomplete.aspx?idnsx=" + eeer);
        }
    }
