...
{
    public partial class Ttbaogia : System.Web.UI.Page
    {
        ...
        private void LoadDataToDropDownList()
        {
            ...
            //DropDownList1.DataTextField = "TenSX"; 
            //DropDownList1.DataValueField = "TenSX"; 
        ...
        protected void load1(string madon)
        {
            ...
            string connectionstring = ConfigurationManager.ConnectionStrings["Tbecon"].ConnectionString;
            SqlConnection conn = new SqlConnection(connectionstring);
            string sql = "select  * from PhieuXuatkho where Madon=@madon";
            SqlCommand cmd = new SqlCommand(sql, conn);
            ...
            while (reader.Read())
            {
                TextBox3.Text = reader["Madon"].ToString();
                TextBox4.Text = reader["Req"].ToString();
                TextBox5.Text = reader["Tieude"].ToString();
                TextBox6.Text = reader["SoEvent"].ToString();
                TextBox7.Text = reader["Subject"].ToString();
                TextBox2.Text = reader["to"].ToString();
                TextBox12.Text = reader["Cc"].ToString();
                TextBox8.Text = reader["Date"].ToString();
                TextBox9.Text = reader["Contactor"].ToString();
                TextBox10.Text = reader["Mobile"].ToString();
                TextBox11.Text = reader["Chuky"].ToString();
            }
            reader.Close();
            conn.Close();
        }
        ...
        protected void capnhat_Click(object sender, EventArgs e)
        {
            ...
            String datet = DateTime.Now.ToString("yyyy-MM-ddTHH:mm:ss");
                using (var cnn = new SqlConnection(System.Web.Configuration.WebConfigurationManager.ConnectionStrings["Tbecon"].ToString()))
                    {
                        using (var conn = new SqlDataAdapter("Insert PhieuXuatkho values('" + madon + "',N'" + TextBox4.Text.Trim() + "',N'" + TextBox5.Text.Trim() + "',N'" + TextBox6.Text.Trim() + "',N'" + TextBox7.Text.Trim() + "',N'" + TextBox2.Text.Trim() + "',N'" + TextBox12.Text.Trim() + "','" + datet + "',N'" + TextBox9.Text.Trim() + "',N'" + TextBox10.Text.Trim() + "',N'" + TextBox11.Text.Trim() + "','" + masp + "','" + RFID.Text.Trim() + "')", cnn))
                        {
                            var data = new DataTable();
                            conn.Fill(data);
                        }
                    }
                    Label2.Text = "Chúc Mừng Bạn Thêm Báo Giá Thành Công";
                    Response.Redirect("ViewBaoGia.aspx?idnsx=" + madon); 
          }
    }
}