...
{
    public partial class AddNhaSanXuat : System.Web.UI.Page
    {
        ...
        public void load1()
        {
            ...
            if (Label10.Text.Equals("muahang") || Label10.Text.Equals("giaonhan") || Label10.Text.Equals("it") || Label10.Text.Equals("thukho") || Label10.Text.Equals("thuquy") || Label10.Text.Equals("nvhcns"))
            {
                Response.Redirect("~/Admin/Chantruycap.aspx");
            }
        ...
        protected void load()
        {
            ...
            string connectionstring = ConfigurationManager.ConnectionStrings["Tbecon"].ConnectionString;
            SqlConnection conn = new SqlConnection(connectionstring);
            SqlCommand cmd = new SqlCommand("select * from NhaSanXuat ", conn);
            SqlDataAdapter adapter;
            adapter = new SqlDataAdapter(cmd);
            DataSet dt = new DataSet();
            adapter.Fill(dt, "NhaSanXuat");
            Repeater1.DataSource = dt.Tables["NhaSanXuat"];
            Repeater1.DataBind();
        }
        ...
        protected void gui_Click(object sender, EventArgs e)
        {
            ...
            if (tenNSX.Text.Equals(""))
            {
                Label3.Visible = true;
                Label3.Text = "Tên NSX Không Được Bỏ Trống";
                tenNSX.Focus();
            }
            if (diachi.Text.Equals(""))
             {
                 Label6.Visible = true;
                 Label6.Text = "Địa Chỉ Không Được Bỏ Trống";
                 diachi.Focus();
             }
            ...
            if (sdt.Text.Equals(""))
             {
                 Label5.Visible = true;
                 Label5.Text = "Số Điện Thoại Không Được Bỏ Trống";
                 sdt.Focus();
             }
            ...
            {
                Label1.Visible = true;
                Label1.Text = "Email Không Được Bỏ Trống";
                email.Focus();
            }
            else
            {
                using (var cnn = new SqlConnection(System.Web.Configuration.WebConfigurationManager.ConnectionStrings["Tbecon"].ToString()))
                    {
                        using (var conn = new SqlDataAdapter("Insert NhaSanXuat values('','" + tenNSX.Text.Trim() + "','" + diachi.Text.Trim() + "','" + sdt.Text.Trim() + "','" + email.Text.Trim() + "')", cnn))
                        {
                            var data = new DataTable();
                            conn.Fill(data);
                        }
                    }
                    Label2.Text = "Chúc Mừng Bạn Thêm Nhà Sản Xuất Thành Công";
                    load(); 
              }                         
        }
