...
namespace Banhang.Admin
{
    public partial class PhieuXuatKho : System.Web.UI.Page
    {
        ...
        protected void load()
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT DISTINCT Mahoadon,Date from Xuatkho", conn);
            SqlDataAdapter adapter;
            adapter = new SqlDataAdapter(cmd);
            DataSet dt = new DataSet();
            adapter.Fill(dt, "Xuatkho");
            Repeater1.DataSource = dt.Tables["Xuatkho"];
            Repeater1.DataBind();
        }
        public void load1()
        {
        ...
        private void LoadDataToDropDownList1()
        {
            ...
            xuatct.DataTextField = "Tenkho"; //Text hiển thị
            xuatct.DataValueField = "Tenkho"; //Giá trị khi chọn
            xuatct.DataBind();
        }
        ...
        protected void gui_Click(object sender, EventArgs e)
        {
            ...
            if (nguoinhan.Text.Equals(""))
            {
                Lnguoinhan.Visible = true;
                Lnguoinhan.Text = "Người Nhận Không Được Bỏ Trống";
                nguoinhan.Focus();
            }
            if (Sochungtu.Text.Equals(""))
            {
                LSochungtu.Visible = true;
                LSochungtu.Text = "Số Chứng Từ Không Được Bỏ Trống";
                Sochungtu.Focus();
            }
            else
            {
                ...
                {
                    SqlCommand cmd = new SqlCommand("select * from Xuatkho where Mahoadon='" + madon.Text.Trim() + "' ", cnn);
                    SqlDataAdapter adapter;
                    adapter = new SqlDataAdapter(cmd);
                    DataSet dt = new DataSet();
                    ...
                    {
                        Label2.Text = "Trùng Mã Xuất Kho! Vui Lòng Chọn Lại Mã";
                    }
                    else
                    {
                        string date = DateTime.Now.ToString("ddMMyyyy-HHmm");
                        madon.Text = "S" + date;
                        using (var conn = new SqlDataAdapter("Insert Xuatkho values('" + madon.Text.Trim() + "',N'" + tencty.Text.Trim() + "','" + datet + "',1,1,N'" + diachi.Text.Trim() + "',N'" + xuatct.Text.Trim() + "','" + nguoinhan.Text.Trim() + "','" + Xuattaikho.Text.Trim() + "','',N'" + Sochungtu.Text.Trim() + "','mamacdinh','','','')", cnn))
                        {
                            var data = new DataTable();
                            conn.Fill(data);
                            Label2.Text = "Chúc Mừng Bạn Thêm Phiếu Xuất Kho Thành Công";
                            load();
                            Response.Redirect("XuatSanPham.aspx?idnsx=" + madon.Text.Trim());
                        }
                    }
                 }                   
              }                         
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
           ...
           using (var cnn = new SqlConnection(System.Web.Configuration.WebConfigurationManager.ConnectionStrings["Tbecon"].ToString()))
           {
               string ID1 = "mamacdinh";
                using (var conn2 = new SqlDataAdapter("Delete from Xuatkho where MaSP='" + ID1 + "' ", cnn))
                 {
                       var data = new DataTable();
                       conn2.Fill(data);
                 }
