...
namespace TBE.Admin
{
    public partial class Giaothatbai : System.Web.UI.Page
    {
        ...
        public void load1()
        {
            ...
            else if (Label3.Text.Equals("kho") || Label3.Text.Equals("giamdoc"))
            {
                ...
                var sqlda = new SqlDataAdapter("SELECT * from Xuatkho where StatusPXK=N'Giao Thất Bại' ORDER BY Date DESC", sqlcon);
            ...
            {
                var sqlcon = new SqlConnection(ConfigurationManager.ConnectionStrings["Tbecon"].ToString());
                var sqlda = new SqlDataAdapter("SELECT * from Xuatkho where StatusPXK=N'Giao Thất Bại' and Nguoigiaohanh=N'" + hotenad.Text + "' ORDER BY Date DESC", sqlcon);
                var dt1 = new DataTable();
                sqlda.Fill(dt1);
                lvCustomers.DataSource = dt1;
                lvCustomers.DataBind();
