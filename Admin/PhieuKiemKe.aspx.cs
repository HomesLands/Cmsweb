...
{
    public partial class PhieuKiemKe : System.Web.UI.Page
    {
        ...
        protected void load()
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT DISTINCT Mahoadon,Date from KiemKe", conn);
            SqlDataAdapter adapter;
            adapter = new SqlDataAdapter(cmd);
            DataSet dt = new DataSet();
            adapter.Fill(dt, "KiemKe");
            Repeater1.DataSource = dt.Tables["KiemKe"];
            Repeater1.DataBind();
        }
        public void load1()
        {
        ...
        private void LoadDataToDropDownList1()
        {
            ...
            DropDownList2.DataTextField = "DiaChi"; //Text hiển thị
            DropDownList2.DataValueField = "DiaChi"; //Giá trị khi chọn
            DropDownList2.DataBind();

            Tenkho.DataSource = dataTable;
            Tenkho.DataTextField = "Tenkho"; //Text hiển thị
            Tenkho.DataValueField = "Tenkho"; //Giá trị khi chọn
            Tenkho.DataBind();
        }
        protected void gui_Click(object sender, EventArgs e)
        {
            ...
            if (Nguoikiemke.Text != "")
            {
                LNguoikiemke.Visible = false;
            }
            if (madon.Text.Equals(""))
            {
                Lmadon.Visible = true;
                Lmadon.Text = "Mã Kiểm Kê Không Được Bỏ Trống";
                madon.Focus();
            }
            if (tencty.Text.Equals(""))
             {
                 Ltencty.Visible = true;
                 Ltencty.Text = "Tên Công Ty Không Được Bỏ Trống";
                 tencty.Focus();
             }
            if (Nguoikiemke.Text.Equals(""))
            {
                LNguoikiemke.Visible = true;
                LNguoikiemke.Text = "Người Kiểm Kê Không Được Bỏ Trống";
                Nguoikiemke.Focus();
            }
            ...
            else
            {
                ...
                {
                    SqlCommand cmd = new SqlCommand("select * from KiemKe where Mahoadon='" + madon.Text.Trim() + "' ", cnn);
                    SqlDataAdapter adapter;
                    adapter = new SqlDataAdapter(cmd);
                    DataSet dt = new DataSet();
                    ...
                    {
                        Label2.Text = "Trùng Mã Kiểm Kê! Vui Lòng Chọn Lại Mã";
                    }else
                    {
                        string date = DateTime.Now.ToString("ddMMyyyy-HHmm");
                        madon.Text = "KK" + date;
                        using (var conn = new SqlDataAdapter("Insert KiemKe values('" + madon.Text.Trim() + "',N'" + tencty.Text.Trim() + "',N'" + Tenkho.Text.Trim() + "','" + datet + "',1,N'" + diachicty.Text.Trim() + "',N'" + DropDownList2.Text.Trim() + "',N'" + Nguoikiemke.Text.Trim() + "',N'" + Lydokiem.Text.Trim() + "','mamacdinh','','','')", cnn))
                        {
                            var data = new DataTable();
                            conn.Fill(data);
                            Label2.Text = "Chúc Mừng Bạn Thêm Phiếu Kiểm Kho Thành Công";
                            load();
                            Response.Redirect("XuatKiemKe.aspx?idnsx=" + madon.Text.Trim());
                        }
                    }
                }                    
              }                         
              ...
              using (var cnn = new SqlConnection(System.Web.Configuration.WebConfigurationManager.ConnectionStrings["Tbecon"].ToString()))
                    {
                        ...
                        using (var conn2 = new SqlDataAdapter("Delete from KiemKe where MaSP='" + ID1 + "' ", cnn))
                            {
                                var data = new DataTable();
                                conn2.Fill(data);
                            }
