...
namespace Banhang.Admin
{
    public partial class ListUser : System.Web.UI.Page
    {
        ...
        protected void load()
        {
            ...
            SqlCommand cmd = new SqlCommand("select * from Admin where not UserName='admin'  ", conn);
        ...
        private void LoadDataToDropDownList1()
        {
            ...
            SqlDataAdapter adapter = new SqlDataAdapter("Select DISTINCT Bophan From Admin", connectionstring);
            adapter.Fill(dataTable);
            DropDownList1.DataSource = dataTable;
            DropDownList1.DataTextField = "Bophan"; //Text hiển thị
            DropDownList1.DataValueField = "Bophan"; //Giá trị khi chọn
        ...
        public void timkiem1()
        {
            string constr = ConfigurationManager.ConnectionStrings["Tbecon"].ConnectionString;
            SqlConnection conn1 = new SqlConnection(constr);
            SqlCommand cmd = new SqlCommand("select * from Admin where HoTen like N'%" + tennv.Text.Trim() + "%' ", conn1);
            SqlDataAdapter adapter;
            adapter = new SqlDataAdapter(cmd);
            DataSet dt = new DataSet();
            adapter.Fill(dt, "Admin");
        ...
        public void timkiem2()
        {
            string constr = ConfigurationManager.ConnectionStrings["Tbecon"].ConnectionString;
            SqlConnection conn1 = new SqlConnection(constr);
            SqlCommand cmd = new SqlCommand("select * from Admin where Bophan like N'%" + DropDownList1.Text.Trim() + "%'   ", conn1);
            SqlDataAdapter adapter;
            adapter = new SqlDataAdapter(cmd);
            DataSet dt = new DataSet();
            adapter.Fill(dt, "Admin");
        ...
        public void timkiem3()
        {
            string constr = ConfigurationManager.ConnectionStrings["Tbecon"].ConnectionString;
            SqlConnection conn1 = new SqlConnection(constr);
            SqlCommand cmd = new SqlCommand("select * from Admin where DT like N'%" + sdt.Text.Trim() + "%'    ", conn1);
            SqlDataAdapter adapter;
            adapter = new SqlDataAdapter(cmd);
            DataSet dt = new DataSet();
            adapter.Fill(dt, "Admin");
        ...
        {    
            ...
            var sqlcon = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["Tbecon"].ToString());
            if (tennv.Text != "")
            {
                (lvCustomers.FindControl("DataPager1") as DataPager).SetPageProperties(e.StartRowIndex, e.MaximumRows, false);
                this.timkiem1();
            }
            else if (DropDownList1.SelectedValue.ToString().Trim() != "")
            {
                (lvCustomers.FindControl("DataPager1") as DataPager).SetPageProperties(e.StartRowIndex, e.MaximumRows, false);
                this.timkiem2();
            }
        ...
        protected void Button2_Click(object sender, EventArgs e)
        {
            ...
            else if (tennv.Text == "" || DropDownList1.Text == "" || sdt.Text == "")
            {
