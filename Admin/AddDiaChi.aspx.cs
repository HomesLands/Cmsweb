...
{
    public partial class AddDiaChi : System.Web.UI.Page
    {
        ...
        protected void load()
        {
            ...
            SqlCommand cmd = new SqlCommand("select * from DiachiKho", conn);
            SqlDataAdapter adapter;
            adapter = new SqlDataAdapter(cmd);
            DataSet dt = new DataSet();
            adapter.Fill(dt, "DiachiKho");
            Repeater1.DataSource = dt.Tables["DiachiKho"];
            Repeater1.DataBind();
        }
        ...
        protected void gui_Click(object sender, EventArgs e)
        {
            ...
            if (tendiachi.Text != "")
            {
                Ltendiachi.Visible = false;
            }
            if (diachikho.Text != "")
            {
                Ldiachikho.Visible = false;
            }       
            if (tendiachi.Text.Equals(""))
            {
                Ltendiachi.Visible = true;
                Ltendiachi.Text = "Tên Địa Chỉ Không Được Bỏ Trống";
                tendiachi.Focus();
            }
            if (diachikho.Text.Equals(""))
             {
                 Ldiachikho.Visible = true;
                 Ldiachikho.Text = "Địa Chỉ kho Không Được Bỏ Trống";
                 diachikho.Focus();
             }
            else
            {
                using (var cnn = new SqlConnection(System.Web.Configuration.WebConfigurationManager.ConnectionStrings["Tbecon"].ToString()))
                    {
                        using (var conn = new SqlDataAdapter("Insert DiachiKho values(N'" + tendiachi.Text.Trim() + "',N'" + diachikho.Text.Trim() + "')", cnn))
                        {
                            var data = new DataTable();
                            conn.Fill(data);
                        }
                    }
                    Label2.Text = "Chúc Mừng Bạn Thêm Địa Chỉ Thành Công";
                    load(); 
              }
