    ...
    public partial class Nhapkho : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    load();
                    Label12.Text = "Welcome To " + " " + Session["ADminID"];
                    txt_autocomplete.Attributes.Add("onkeypress", "return clickButton(event,'" + Button3.ClientID + "')");
                    TextBox2.Attributes.Add("onkeypress", "return clickButton(event,'" + Button4.ClientID + "')");
                    LoadDataToDropDownList();
                    Label3.Visible = false;
                    Label8.Visible = false;
                    Label11.Visible = false;
                    trangthairfid.Visible = false;
        ...
        public void load()
        {
            ...
            while (reader.Read())
            {
                Label11.Text = reader["Manager"].ToString();
                hotenad.Text = reader["HoTen"].ToString();
            }
            reader.Close();
            conn1.Close();
            if (Label11.Text.Equals("khach"))
            {
                Response.Redirect("~/Admin/Chantruycap.aspx");
            }
            if (Label11.Text.Equals("ketoan"))
            {
                Response.Redirect("~/Admin/Chantruycap.aspx");
            }
            if (Label11.Text.Equals("muahang") || Label3.Text.Equals("giaonhan") || Label11.Text.Equals("it") || Label11.Text.Equals("thukho") || Label11.Text.Equals("thuquy") || Label11.Text.Equals("nvhcns") || Label11.Text.Equals("nvpda"))
            {
                Response.Redirect("~/Admin/Chantruycap.aspx");
            }
            if (Label11.Text.Equals("chihuytruong"))
            {
                Response.Redirect("~/Admin/Chantruycap.aspx");
            }
            if (Label11.Text.Equals("ppda") || Label11.Text.Equals("tpkehoach") || Label11.Text.Equals("tpda") || Label11.Text.Equals("tpmh") || Label11.Text.Equals("ktt") || Label11.Text.Equals("tpns"))
            {
                Response.Redirect("~/Admin/Chantruycap.aspx");
            }
            if (Label11.Text.Equals("nhanvien") || Label11.Text.Equals("gsat") || Label11.Text.Equals("gsd") || Label11.Text.Equals("gsck") || Label11.Text.Equals("gs"))
            {
                Response.Redirect("~/Admin/Chantruycap.aspx");
            }
            if (Label11.Text.Equals("thohan") || Label11.Text.Equals("thocokhi") || Label11.Text.Equals("baotridien") || Label11.Text.Equals("baotricokhi") || Label11.Text.Equals("QSQC"))
            {
                Response.Redirect("~/Admin/Chantruycap.aspx");
            }
        }
        ...
        protected void load1()
        {
            ...
            {
                Label3.Visible = false;
                if (txt_autocomplete.Text.Length <= 8)
                {
                    string connectionstring = ConfigurationManager.ConnectionStrings["Tbecon"].ConnectionString;
                    SqlConnection conn = new SqlConnection(connectionstring);
                    SqlCommand cmd = new SqlCommand("select * from SanPhamTBE where MaSP='" + txt_autocomplete.Text.Trim() + "'  ", conn);
                    SqlDataAdapter adapter;
                    adapter = new SqlDataAdapter(cmd);
                    DataSet dt = new DataSet();
                    ...
                    {
                        Repeater1.DataSource = dt.Tables["SanPhamTBE"];
                        Repeater1.DataBind();
                        SqlConnection conn1 = new SqlConnection(connectionstring);
                        string sql = "select  * from SanPhamTBE where Masp='" + txt_autocomplete.Text + "'  ";
                        SqlCommand cmd1 = new SqlCommand(sql, conn1);
                        cmd1.Parameters.AddWithValue("@masp", txt_autocomplete.Text);
                        conn1.Open();
                        SqlDataReader reader = cmd1.ExecuteReader();
                        while (reader.Read())
                        {
                            TextBox3.Text = reader["MaModel"].ToString();
                            tensp.Text = reader["TenSP"].ToString();
                          //  DropDownList1.Text = reader["MaSX"].ToString();
                            dongia.Text = reader["DonGia"].ToString();
                            slton.Text = reader["SLTon"].ToString();
                            txtPlaces.Text = reader["Diachi"].ToString();
                    ...
                    {
                        Label8.Text = "Mã sản phẩm không có";
                ...
                {
                    Label8.Text = "Mã sản phẩm không chính xác";
                    txt_autocomplete.Text = "";
                    txt_autocomplete.Focus();
                }
            }
            ...
            {
                Label3.Visible = true;
                Label3.Text = "Mã Sản Phẩm Không Được Bỏ Trống";
                txt_autocomplete.Focus();
            }                
        }
        ...
        {
            //Label1.Text = "Bạn vừa chọn: " + DropDownList1.SelectedValue;
            string connectionstring = ConfigurationManager.ConnectionStrings["Tbecon"].ConnectionString;
            SqlConnection conn=new SqlConnection(connectionstring);
            DataTable dataTable = new DataTable();
        ...
        {           
            ...
            else
            {
                ...
                {
                    SqlCommand cmd = new SqlCommand("select * from SanPhamTBE where MaRFID='" + TextBox2.Text.Trim() + "' ", cnn);
                    SqlDataAdapter adapter;
                    adapter = new SqlDataAdapter(cmd);
                    DataSet dt = new DataSet();
                    adapter.Fill(dt, "SanPhamTBE");
                    ...
                    while (reader.Read())
                    {
                        Label11.Text = reader["trangthai"].ToString();
                    }
                    reader.Close();
                    cnn.Close();
                    if (TextBox2.Text.Trim() == "")
                    {
                        Label8.Text = "Vui Lòng Điền RFID";
                        TextBox2.Text = "";
                        TextBox2.Focus();
                    }
                    else if (adapter.Fill(dt) > 0)
                    {
                        Label8.Text = "Trùng RFID";
                        load1();
                        TextBox2.Text = "";
                        TextBox2.Focus();
                    }
                    ...
                    {
                        if (TextBox2.Text.Length != 22 || TextBox2.Text.IndexOf("E") > 0)
                        {
                            Label8.Text = "Mã RFID gồm 22 Số";
                            TextBox2.Text = "";
                            TextBox2.Focus();
                        }
                        else
                        {
                            //string ID1 = "chuaxacdinh";
                            TextBox2.Text = TextBox2.Text.Substring(0, 22);
                            //using (var conn2 = new SqlDataAdapter("Delete from SanPhamTBE where MaSP='" + txt_autocomplete.Text.Trim() + "' and MaRFID='" + ID1 + "' ", cnn))
                            //{
                            //    var data = new DataTable();
                            //    conn2.Fill(data);
                            //}
                            using (var conn = new SqlDataAdapter("Insert SanPhamTBE values('" + txt_autocomplete.Text.Trim() + "',N'" + DropDownList1.Text.Trim() + "','" + TextBox2.Text.Trim() + "','" + TextBox3.Text.Trim() + "',N'" + tensp.Text.Trim() + "','" + date + "','" + slton.Text.Trim() + "','" + dongia.Text.Trim() + "','" + txt_autocomplete.Text.Trim() + "',N'" + txtPlaces.Text.Trim() + "',N'" + ghichu.Text.Trim() + "','action')", cnn))
                            {
                                var data = new DataTable();
                                conn.Fill(data);
                            }
                            Label8.Text = "Thêm Thành Công";
        ...
        protected void Repeater1_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            if (e.CommandName == "delete")
            {
                string connectionstring = ConfigurationManager.ConnectionStrings["Tbecon"].ConnectionString;
                SqlConnection conn = new SqlConnection(connectionstring);
                SqlCommand SqlCmd = new SqlCommand("delete SanPhamTBE where MaRFID=@ID", conn);
                SqlCmd.Parameters.Add("@ID", SqlDbType.VarChar).Value = e.CommandArgument;
                try
                {
                    if (e.CommandArgument.Equals("chuaxacdinh"))
                    {
                        Label8.Visible = true;
                        Label8.Text = "Không thể xóa";
                    }
                    else
                    {
                        conn.Open();
